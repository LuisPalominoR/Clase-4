...
```{r}
# filtering 
spain=countriesJson[countriesJson$COUNTRY=='Spain',]
```
...
```{r}
# clipping
citiesSpain = st_intersection(spain,citiesJson)
riversSpain = st_intersection(spain,riversJson)
```

```{r}
spain_2062 = st_transform(spain, "epsg:2062")
st_crs(spain_2062)
```

```{r warning=FALSE}
st_centroid(spain_2062)
```

```{r}
citiesspain_2062=st_transform(citiesSpain, "epsg:2062")
riversspain_2062=st_transform(riversSpain, "epsg:2062")
```


```{r warning=FALSE}
base=ggplot() 
layer1a=base + geom_sf(data=spain_2062, fill='white')
layer2a=layer1a + geom_sf(data=st_centroid(spain_2062),color='red',size=10)
layer3a=layer2a + geom_sf(data=citiesspain_2062,color='green',size=5,shape='+')
layer4a=layer3a + geom_sf(data=riversspain_2062,color='blue',linewidth=0.5)
finalMap2=layer4a+  coord_sf(datum = st_crs(spain_2062))
finalMap2
